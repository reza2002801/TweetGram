...
public class MessageMethods {
    ...
    public static void sendMessage(message mess,String UserName) throws IOException {
        ...
        for(int i=0;i<currentuser.getUserchats().size();i++){
            if(currentuser.getUserchats().get(i).getContactname().equals(UserName)){
                ...
                AccountsMethods.findAccount(accounts,LoginController.USERNAME).getUserchats().get(i).getMessages().add(mess);
            }
        }
    ...
    public static Boolean existchat(List<User> e, String contactname) throws IOException {
...
//        List<user> accounts=new Filemethods().loadFromFile();
        if (AccountsMethods.findAccount(e,LoginController.USERNAME).getUserchats().size()==0){
            return false;
        }else{
            for (int i = 0; i< AccountsMethods.findAccount(e,LoginController.USERNAME).getUserchats().size(); i++){
                if (AccountsMethods.findAccount(e,LoginController.USERNAME).getUserchats().get(i).getContactname().equals(contactname)){
                    return true;
                }
            }
        }
