...
import static controllers.AccountsMethods.findAccount;
...
//        r.close();
        ...
        for(int i = 0; i< AccountsMethods.findAccount(a,LoginController.USERNAME).getFollowers().size(); i++){
            AccountsMethods.findAccount(a,AccountsMethods.findAccount(a,LoginController.USERNAME).getFollowers().get(i)).getFollowingstweet().add(n);
...
//        logger.debug("in add_com_to_follower from MainPage class on values"+e+"and"+t+"and"+b);
        ...
        for(int i=0;i<f.size();i++){
            AccountsMethods.findAccount(e,f.get(i)).getFollowingstweet().add(t);
