...
public class Filemethods {
    ...
    public List<Change> loadFromChange() throws IOException {
        Gson g=new GsonBuilder().setPrettyPrinting().create();
        Reader r= Files.newBufferedReader(Paths.get("Change.json"));
        List<Change> e = g.fromJson(r, new TypeToken<List<Change>>() {}.getType());
        r.close();
        return e;
    }
    public void saveToChange(List<Change> ch) throws IOException {
//        logger.debug("in saveToFileGroup from Filemethods class on values"+e);
        Gson g=new GsonBuilder().setPrettyPrinting().create();
        Writer w=new FileWriter("Change.json");
        g.toJson(ch,w);
        w.close();
    }
